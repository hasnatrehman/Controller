	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 CalculateContrast( float contrastValue, float4 colorTarget )
			{
				...
				return mul( float4x4( contrastValue,0,0,t, 0,contrastValue,0,t, 0,0,contrastValue,t, 0,0,0,1 ), colorTarget );
					...
					float2 voronoihash58_g892( float2 p )
					{
					...
					float voronoi58_g892( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash58_g892( n + g );
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				float2 break26_g894 = uv_BaseMap;
				...
				float4 tex2DNode7_g893 = tex2D( _BaseMap, ( ( appendResult14_g894 + appendResult13_g894 ) + Parallaxe494 ) );
				float4 lerpResult53_g893 = lerp( _BaseColor , ( ( _BaseColor * tex2DNode7_g893 ) * _BaseColor.a ) , _BaseColor.a);
				float clampResult27_g893 = clamp( _Saturation , -1.0 , 100.0 );
				float3 desaturateInitialColor29_g893 = lerpResult53_g893.rgb;
				float desaturateDot29_g893 = dot( desaturateInitialColor29_g893, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar29_g893 = lerp( desaturateInitialColor29_g893, desaturateDot29_g893.xxx, -clampResult27_g893 );
				float4 temp_output_843_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g893 , 0.0 ));
				...
				float2 texCoord114_g852 = IN.ase_texcoord8.xyz.xy * temp_cast_2 + float2( 0,0 );
				...
				float2 texCoord105_g852 = IN.ase_texcoord8.xyz.xy * temp_cast_3 + _OffsetFBDetailsNormal;
				...
				float4 BaseColorRGB403 = temp_output_843_0;
				...
				float staticSwitch28_g871 = tex2DNode27_g871.r;
				#endif
				float BaseColorAlpha46 = (( _InvertABaseColor1 )?( ( 1.0 - tex2DNode7_g893.a ) ):( tex2DNode7_g893.a ));
				...
				float4 lerpResult365 = lerp( temp_output_843_0 , AlbedoDetail153 , Mask158);
				...
				float2 break26_g896 = uv_BumpMap;
				float2 appendResult14_g896 = (float2(( break26_g896.x * GlobalTilingX11 ) , ( break26_g896.y * GlobalTilingY8 )));
				float2 appendResult13_g896 = (float2(( break26_g896.x + GlobalOffsetX10 ) , ( break26_g896.y + GlobalOffsetY9 )));
				float3 unpack4_g895 = UnpackNormalScale( tex2D( _BumpMap, ( ( appendResult14_g896 + appendResult13_g896 ) + Parallaxe494 ) ), _BumpScale );
				unpack4_g895.z = lerp( 1, unpack4_g895.z, saturate(_BumpScale) );
				float3 tex2DNode4_g895 = unpack4_g895;
				float3 temp_output_844_0 = tex2DNode4_g895;
				...
				float3 lerpResult137 = lerp( temp_output_844_0 , temp_output_740_0 , Mask158.rgb);
				float3 lerpResult205 = lerp( temp_output_844_0 , BlendNormal( temp_output_844_0 , temp_output_740_0 ) , Mask158.rgb);
				...
				float2 break26_g898 = uv_EmissionMap;
				float2 appendResult14_g898 = (float2(( break26_g898.x * GlobalTilingX11 ) , ( break26_g898.y * GlobalTilingY8 )));
				float2 appendResult13_g898 = (float2(( break26_g898.x + GlobalOffsetX10 ) , ( break26_g898.y + GlobalOffsetY9 )));
				float4 temp_output_5_0_g897 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g898 + appendResult13_g898 ) + Parallaxe494 ) ) );
				float4 temp_output_845_0 = ( temp_output_5_0_g897 * _EmissionIntensity );
				float4 temp_cast_16 = (0.0).xxxx;
				float4 lerpResult394 = lerp( temp_output_845_0 , temp_cast_16 , Mask158);
				float3 worldRefl28_g899 = normalize( reflect( -WorldViewDirection, float3( dot( tanToWorld0, (( _UseMainNormalMapasNormalDirection )?( Normal27 ):( NormalPuddles459 )) ), dot( tanToWorld1, (( _UseMainNormalMapasNormalDirection )?( Normal27 ):( NormalPuddles459 )) ), dot( tanToWorld2, (( _UseMainNormalMapasNormalDirection )?( Normal27 ):( NormalPuddles459 )) ) ) ) );
				float4 texCUBENode27_g899 = texCUBElod( _Cubemap, float4( worldRefl28_g899, _BlurReflection) );
				float clampResult39_g899 = clamp( _ReflectionIntensity , 0.0 , 100.0 );
				float4 Emission110 = ( (( _UseEmissionFromMainProperties )?( temp_output_845_0 ):( lerpResult394 )) + ( ( texCUBENode27_g899 * ( texCUBENode27_g899.a * clampResult39_g899 ) * _ColorCubemap ) * Mask158 ) );
				...
				float time58_g892 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g892 = 0;
				float2 texCoord55_g892 = IN.ase_texcoord8.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords58_g892 = texCoord55_g892 * _Tiling;
				float2 id58_g892 = 0;
				float2 uv58_g892 = 0;
				float fade58_g892 = 0.5;
				float voroi58_g892 = 0;
				float rest58_g892 = 0;
				for( int it58_g892 = 0; it58_g892 <2; it58_g892++ ){
				voroi58_g892 += fade58_g892 * voronoi58_g892( coords58_g892, time58_g892, id58_g892, uv58_g892, 0,voronoiSmoothId58_g892 );
				rest58_g892 += fade58_g892;
				coords58_g892 *= 2;
				fade58_g892 *= 0.5;
				}//Voronoi58_g892
				voroi58_g892 /= rest58_g892;
				float2 myVarName60_g892 = id58_g892;
				float2 panner63_g892 = ( 1.0 * _Time.y * temp_cast_21 + ( step( voroi58_g892 , 0.1 ) * myVarName60_g892 ));
				float RainDotsRoughness606 = ( saturate( ( tex2D( _GradientTex, panner63_g892 ).g * step( voroi58_g892 , (0.0 + (_Size - 0.0) * (0.05 - 0.0) / (1.0 - 0.0)) ) ) ) * _Intensity );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float FlipBookTiling100_g852 = _FlipBTilingNormal;
				float2 temp_cast_2 = (FlipBookTiling100_g852).xx;
				float2 texCoord114_g852 = IN.ase_texcoord4.xyz.xy * temp_cast_2 + float2( 0,0 );
				float2 appendResult124_g852 = (float2(frac( texCoord114_g852.x ) , frac( texCoord114_g852.y )));
				float XColumns119_g852 = _XColumnsYRowsZSpeedWStrartFrameNormal.x;
				float YRows118_g852 = _XColumnsYRowsZSpeedWStrartFrameNormal.y;
				float ZSpeed121_g852 = ( ( _XColumnsYRowsZSpeedWStrartFrameNormal.x * _XColumnsYRowsZSpeedWStrartFrameNormal.y ) * _XColumnsYRowsZSpeedWStrartFrameNormal.z );
				...
				float3 tex2DNode132_g852 = unpack132_g852;
				float2 temp_cast_3 = (( FlipBookTiling100_g852 / _ScaleFBDetailsNormal )).xx;
				float2 texCoord105_g852 = IN.ase_texcoord4.xyz.xy * temp_cast_3 + _OffsetFBDetailsNormal;
				float cos111_g852 = cos( _FBDetailsNormal );
				float sin111_g852 = sin( _FBDetailsNormal );
				float2 rotator111_g852 = mul( texCoord105_g852 - float2( 0.5,0.5 ) , float2x2( cos111_g852 , -sin111_g852 , sin111_g852 , cos111_g852 )) + float2( 0.5,0.5 );
				float2 break112_g852 = rotator111_g852;
				...
				float4 lerpResult394 = lerp( temp_output_845_0 , temp_cast_12 , Mask158);
				...
				float3 lerpResult205 = lerp( temp_output_844_0 , BlendNormal( temp_output_844_0 , temp_output_740_0 ) , Mask158.rgb);
				float3 Normal27 = (( _BlendMainNormal )?( lerpResult205 ):( lerpResult137 ));
				float3 worldRefl28_g899 = normalize( reflect( -ase_worldViewDir, float3( dot( tanToWorld0, (( _UseMainNormalMapasNormalDirection )?( Normal27 ):( NormalPuddles459 )) ), dot( tanToWorld1, (( _UseMainNormalMapasNormalDirection )?( Normal27 ):( NormalPuddles459 )) ), dot( tanToWorld2, (( _UseMainNormalMapasNormalDirection )?( Normal27 ):( NormalPuddles459 )) ) ) ) );
				float4 texCUBENode27_g899 = texCUBElod( _Cubemap, float4( worldRefl28_g899, _BlurReflection) );
				float clampResult39_g899 = clamp( _ReflectionIntensity , 0.0 , 100.0 );
				float4 Emission110 = ( (( _UseEmissionFromMainProperties )?( temp_output_845_0 ):( lerpResult394 )) + ( ( texCUBENode27_g899 * ( texCUBENode27_g899.a * clampResult39_g899 ) * _ColorCubemap ) * Mask158 ) );
